...
hime_label_show (char *pszPho, int nPos) {
    ...
    if (*g_himeModMainFuncs.mf_hime_use_custom_theme)
        ...
        snprintf (szTmp, sizeof (szTmp), "<span background=\"" HIME_CURSOR_COLOR_DEFAULT "\">%s</span>",
                  pszPho);
...
hime_zuin_label_show (void) {
    ...
    pszTmp = chewing_bopomofo_String_static (g_pChewingCtx);
    nZuinLen = strlen (pszTmp) / 3;
...
hime_chewing_wrapper_bs (ChewingContext *pCtx) {
    ...
    const char *pszWord = chewing_bopomofo_String_static (pCtx);
    if (pszWord[0] != '\0')
...
int module_init_win (HIME_module_main_functions *pFuncs) {
    ...
    g_himeModMainFuncs.mf_set_chinese_mode ();
...
int module_get_preedit (char *pszStr, HIME_PREEDIT_ATTR himePreeditAttr[], int *pnCursor, int *pCompFlag) {
    ...
    *pnCursor = 0;
    ...
    if (g_himeModMainFuncs.mf_hime_display_on_the_spot_key ()) {
        pszZuinStr = chewing_bopomofo_String_static (g_pChewingCtx);
...
module_feedkey (int xkey, int nKeyState) {
    ...
    chewing_set_ShapeMode (g_pChewingCtx, g_himeModMainFuncs.mf_current_fullwidth_mode ());
