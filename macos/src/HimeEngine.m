...
- (HimeKeyResultType)processKeyCode:(unsigned short)keyCode
    ...
    if (modifiers & HimeModifierShift)
        himeModifiers |= HIME_MOD_SHIFT;
    if (modifiers & HimeModifierControl)
        himeModifiers |= HIME_MOD_CONTROL;
    if (modifiers & HimeModifierAlt)
        himeModifiers |= HIME_MOD_ALT;
    if (modifiers & HimeModifierCapsLock)
        himeModifiers |= HIME_MOD_CAPSLOCK;
